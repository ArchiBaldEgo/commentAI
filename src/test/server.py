...
def label_to_score(label: str, probs: dict | None = None) -> int:
    """Преобразуем метку и уверенность модели в оценку 1–5.

    Логика (учебный пример):
    - если модель уверенно отрицательная (neg >= 0.8)  -> 1
    - если слабый минус (neg < 0.8)                    -> 2
    - если нейтрально                                 -> 3
    - если слабый плюс (pos < 0.8)                     -> 4
    - если модель уверенно положительная (pos >= 0.8) -> 5
    """
    p_neg = (probs or {}).get("neg", 0.0)
    p_pos = (probs or {}).get("pos", 0.0)

    if label == "neg":
        return 1 if p_neg >= 0.8 else 2
    if label == "neu":
        return 3
    if label == "pos":
        return 5 if p_pos >= 0.8 else 4
    return 3
...
def _load_history() -> None:
    ...
    Это нужно только для демонстрации преподавателю: чтобы после
